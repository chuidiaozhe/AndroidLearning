...
public class DountChartView extends View {
    ...
    public DountChartView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context,attrs);
    }

    public DountChartView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int radius = viewWidht > viewHeight ? viewHeight:viewWidht;
        Paint.FontMetrics fontMetrics = getTextPaint().getFontMetrics();
        canvas.translate(viewWidht/2.0f,radius/2.0f);
        float x = radius/2.0f - lineLenght - Math.abs(fontMetrics.top);
        RectF rectF = new RectF(-x,-x,x,x);

        RectF rectF1 = new RectF(-x + outRadius,-x+outRadius,x - outRadius,x-outRadius);
        if(dataList != null && dataList.size() > 0){
            float startAng = startAngle;
            float swippAng = 0.0f;
            canvas.drawCircle(0,0,x ,getBgPaint());
            ...
            for (int i = 0; i <  dataList.size(); i++) {
                ...
                if(isSyncTextColor){
                    ...
                    getTextPaint().setColor(lableTextColor);
                }

                swippAng =  360*dataList.get(i).getPercent();

                getmPaint().setColor(dataList.get(i).getColor());
                canvas.drawArc(rectF,startAng,swippAng,true,getmPaint());
                canvas.drawArc(rectF1,startAngle,360.0f,true,getCenterPaint());



                float angle =  (startAng + swippAng/2.0f) % 360.0f  ;
                if(isDrawLableText) {
                    Path paht = new Path();
                    Path path2 = new Path();
                    paht.addCircle(0, 0, x, Path.Direction.CW);
                    path2.addCircle(0, 0, x + lineLenght, Path.Direction.CW);
                    pathMeasure.setPath(paht, false);
                    pathMeasure2.setPath(path2, false);
//                canvas.drawPath(paht,getLinePaint());
//                canvas.drawPath(path2,getLinePaint());
                    ...
                    if(startAng < 0 && startAng + swippAng > 0){
                        float absStartAng = Math.abs(startAng);
                        float absAng = swippAng - absStartAng;
                        if(absStartAng > absAng){
                            startD = pathMeasure.getLength()*((360 - absStartAng + absAng)/360.0f);
                            startD1 = pathMeasure2.getLength()*((360 - absStartAng + absAng)/360.0f);
                            endD = pathMeasure.getLength();
                            endD1 = pathMeasure2.getLength();
                        }else if(absStartAng < absAng){
                            startD = 0;
                            startD1 = 0;
                            endD = pathMeasure.getLength()*((absAng - absStartAng)/360.0f);
                            endD1 = pathMeasure2.getLength()*((absAng - absStartAng)/360.0f);
                        }else{
                            startD = 0;
                            startD1 = 0;
                            endD = pathMeasure.getLength()*0.01f;
                            endD1 = pathMeasure2.getLength()*0.01f;
                        }
                    }else{
                        if (startAng < 0) {
                            startD = pathMeasure.getLength() * ((360 + startAng) / 360.f);
                            startD1 = pathMeasure2.getLength() * ((360 + startAng) / 360.f);
                        } else {
                            startD = pathMeasure.getLength() * (startAng / 360.f);
                            startD1 = pathMeasure2.getLength() * (startAng / 360.f);
                        }
                        endD = startD + pathMeasure.getLength() * (Math.abs(swippAng) / 360.0f);
                        endD1 = startD1 + pathMeasure2.getLength() * (Math.abs(swippAng) / 360.0f);
                    }


                    Path tem1 = new Path();
                    pathMeasure.getSegment(startD, endD, tem1, true);

                    Path tem2 = new Path();
                    pathMeasure2.getSegment(startD1, endD1, tem2, true);

                    PathMeasure p1 = new PathMeasure(tem1, false);
                    p1.getPosTan(p1.getLength() * 0.5f, pos, null);

//                canvas.drawPath(tem1,getLinePaint());
//                canvas.drawPath(tem2,getLinePaint());

                    PathMeasure p2 = new PathMeasure(tem2, false);
                    p2.getPosTan(p2.getLength() * 0.5f, poss, null);
                    canvas.drawLine(pos[0], pos[1], poss[0], poss[1], getLinePaint());
                    float lineGap = 2;
                    if (angle >= -90 && angle <= 90) {
                        getTextPaint().setTextAlign(Paint.Align.LEFT);
                        canvas.drawLine(poss[0] - lineGap, poss[1], poss[0] + lineHLength, poss[1], getLinePaint());
                        if (chartLableFormat != null) {
                            canvas.drawText(chartLableFormat.formatLable(charData), poss[0] + lineHLength + textPading, poss[1] + (Math.abs(fontMetrics.ascent)) / 2.0f, getTextPaint());
                        } else {
                            canvas.drawText(charData.getBottomLable(), poss[0] + lineHLength + textPading, poss[1] + (Math.abs(fontMetrics.ascent)) / 2.0f, getTextPaint());
                        }
                    } else {
                        canvas.drawLine(poss[0] + lineGap, poss[1], poss[0] - lineHLength, poss[1], getLinePaint());
                        getTextPaint().setTextAlign(Paint.Align.RIGHT);
                        if (chartLableFormat != null) {
                            canvas.drawText(chartLableFormat.formatLable(charData), poss[0] - lineHLength - textPading, poss[1] + fontMetrics.descent / 2.0f, getTextPaint());
                        } else {
                            canvas.drawText(charData.getBottomLable(), poss[0] - lineHLength - textPading, poss[1] + fontMetrics.descent / 2.0f, getTextPaint());
                        }
                    }
                }
                startAng = startAng + swippAng;
            }
        }else{
            canvas.drawCircle(0,0,x ,getBgPaint());
            canvas.drawArc(rectF1,startAngle,360.0f,true,getCenterPaint());
